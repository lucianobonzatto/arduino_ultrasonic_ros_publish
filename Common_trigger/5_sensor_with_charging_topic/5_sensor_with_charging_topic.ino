...
std_msgs::Bool bool_charging;
ros::Publisher pub_bool_charging("/charging_signal", &bool_charging);


/****************************************************************
      SENSORS STATEMENTS
****************************************************************/
#define numINT 5          // Total number of interrupts
...
void setup()
{
  ...
  nh.advertise(pub_bool_charging);
  ...
  pinMode(chargingP, INPUT);    // Set interrupt pin 21 (INT5) as INPUT (sensor 6)


  attachInterrupt(digitalPinToInterrupt(sensorP1), call_INT1, CHANGE );    // ISR for INT1
  attachInterrupt(digitalPinToInterrupt(sensorP3), call_INT3, CHANGE );   // ISR for INT3
  attachInterrupt(digitalPinToInterrupt(sensorP4), call_INT4, CHANGE );   // ISR for INT4
  attachInterrupt(digitalPinToInterrupt(sensorP5), call_INT5, CHANGE );   // ISR for INT5
  attachInterrupt(digitalPinToInterrupt(sensorP6), call_INT6, CHANGE );   // ISR for INT6
  attachInterrupt(digitalPinToInterrupt(chargingP), call_INT2, CHANGE );   // ISR for INT6
...
void loop()
{
  ...
  if (millis() - lastDebugMillis >= debugDelay)
  {
    ...
    range_sensor_3.range = distance[1];
    range_sensor_4.range = distance[2];
    range_sensor_5.range = distance[3];
    range_sensor_6.range = distance[4];
    ...
    byte pinRead = PINE & B00100000;    // Faster ; Read pin 3/PE5
    bool_charging.data = pinRead;
    pub_bool_charging.publish(&bool_charging);
